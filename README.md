...
<details>
  ...
  **키친가이드 도메인 개발**
  * 카테고리, 검색, 정렬 기반 필터링 기능 구현
  * 스크랩 수 표시 및 목록에서 사용자 스크랩 여부 확인 기능 구현
  * 사용자가 스크랩한 키친가이드 목록 마이페이지 내 조회, 스크랩 취소 구현

  **후원(포인트) 도메인 개발**
  * PortOne API 연동을 통한 실거래 기반 포인트 충전 구현
  * 후원 시 포인트 차감 및 레시피 작성자의 누적 수익 반영 로직 구현
  * 사용자가 받은 후원 내역 기반 수익 조회 기능 구현
  * 요청일 · 출금 금액 · 처리 상태(대기/완료) 표시 출금 목록 구현

  **나의 냉장고 도메인 - 메모, 지도 기능 개발**
  * 레시피 상세 메모에서 재료 담기 기능 연동
  * 작성한 장보기 메모 장보기 완료 시 냉장고 기능과 연동해 식재료 등록
  * 카카오 Map API 기반 사용자 위치 중심 지도 표시 기능 구현

  **리뷰 도메인 개발**
  * 별점 및 내용을 포함한 리뷰 작성, 수정, 삭제
  * 마이페이지에서 레시피명 / 내가 남긴 별점 / 내용 / 작성일 등 리뷰정보 표시

  **관리자 페이지 개발**
  * 키친가이드 목록 조회 및 관리 기능
  * 레시피 리뷰 목록 조회 및 관리 기능
