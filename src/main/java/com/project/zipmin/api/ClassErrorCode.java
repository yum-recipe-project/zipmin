...
public enum ClassErrorCode implements Code {
    ...
    CLASS_CREATE_DUPLICATE(HttpStatus.CONFLICT, "클래스 중복 작성 시도"),
    CLASS_ALREADY_ENDED(HttpStatus.FORBIDDEN, "클래스 종료 후 접근 시도"),
    
    CLASS_TARGET_INVALID_INPUT(HttpStatus.BAD_REQUEST, "입력값이 유효하지 않음"),
    CLASS_TARGET_CREATE_FAIL(HttpStatus.BAD_REQUEST, "클래스 교육대상 작성 실패"),
    CLASS_TARGET_READ_LIST_FAIL(HttpStatus.BAD_REQUEST, "클래스 교육대상 목록 조회 실패"),
    // CLASS_TARGET_READ_FAIL(HttpStatus.BAD_REQUEST, "클래스 교육대상 조회 실패"),
    // CLASS_TARGET_UPDATE_FAIL(HttpStatus.BAD_REQUEST, "클래스 교육대상 수정 실패"),
    // CLASS_TARGET_DELETE_FAIL(HttpStatus.BAD_REQUEST, "클래스 교육대상 삭제 실패"),
    ...
    CLASS_APPLY_CREATE_FAIL(HttpStatus.BAD_REQUEST, "클래스 신청 작성 실패"),
    CLASS_APPLY_READ_LIST_FAIL(HttpStatus.BAD_REQUEST, "클래스 신청 목록 조회 실패"),
    CLASS_APPLY_READ_FAIL(HttpStatus.BAD_REQUEST, "클래스 신청 조회 실패"),
    CLASS_APPLY_UPDATE_FAIL(HttpStatus.BAD_REQUEST, "클래스 신청 수정 실패"),
    CLASS_APPLY_DELETE_FAIL(HttpStatus.BAD_REQUEST, "클래스 신청 삭제 실패"),
    CLASS_APPLY_NOT_FOUND(HttpStatus.NOT_FOUND, "해당 클래스 신청을 찾을 수 없음"),
    CLASS_APPLY_DUPLICATE(HttpStatus.CONFLICT, "클래스 신청 중복 작성 시도"),
